...
import com.baizhi.widget.actionsheet.R;
...
public class ActionSheet extends Fragment implements OnClickListener {
	...
	private static final int CANCEL_BUTTON_ID;
    private static final int BG_VIEW_ID;
	...
	public void dismiss() {
		if (mDismissed || getFragmentManager() == null) {
			return;
		}
		mDismissed = true;
	...
	private View createView() {
		...
		mBg.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT,
				LayoutParams.MATCH_PARENT));
		mBg.setBackgroundColor(0xcc000000);
	...
	public void onClick(View v) {
		...
		if (v.getId() != CANCEL_BUTTON_ID && v.getId() != BG_VIEW_ID) {
			if (mListener != null && v instanceof Button) {
				mListener.onOtherButtonClick(this, v.getId() - CANCEL_BUTTON_ID
						- 1,((Button)v).getText().toString());
			}
			isCancel = false;
		}
	}
